...
uint16_t g_exti_pins_used = 0u;
static void allocate_exti(Pin pin)
{
    uint16_t mask = 1u << static_cast<uint8_t>(pin);
    if ((g_exti_pins_used & mask) != 0u)
    {
...
void GPIOHelpers::configure_as_input(Port port, Pin pin, PuPd pupd, IntMode int_mode, OSpeed ospeed)
{
    allocate_pin(port, pin);
    if (int_mode == IntMode::Interrupt)
    {
        allocate_exti(pin);
    }
    enable_clock(port);
    ...
    init.Mode  = (int_mode == IntMode::Interrupt) ? GPIO_MODE_IT_RISING_FALLING : GPIO_MODE_INPUT;
