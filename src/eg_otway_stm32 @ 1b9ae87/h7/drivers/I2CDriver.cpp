...
void I2CDriverBase::start_transfer()
{
    ...
    if (m_queue.size() > 0)
    {
        ...
        if (m_trans.tx_len > 0)
        {
            ...
            HAL_I2C_Master_Seq_Transmit_IT(&m_hi2c, m_trans.address << 1u, &m_trans.buffer[0], m_trans.tx_len, auto_stop);
        }
        else
        {
            HAL_I2C_Master_Seq_Receive_IT(&m_hi2c, m_trans.address << 1u, &m_trans.buffer[0], m_trans.rx_len, I2C_LAST_FRAME);
        }
    }
    else
    {
...
void I2CDriverBase::MasterTxCpltCallback()
{
    if (m_trans.rx_len > 0)
    {
        HAL_I2C_Master_Seq_Receive_IT(&m_hi2c, m_trans.address << 1u, &m_trans.buffer[m_trans.tx_len], m_trans.rx_len, I2C_LAST_FRAME);
    }
    else
    {
        finish_transfer();
