...
{
    // Configure the pin, but not as an interrupt input.
    GPIOHelpers::configure_as_input(m_conf.port, m_conf.pin, m_conf.pupd, IntMode::Polled);
...
{
    // TODO_AC This is not debouncing but straight polling.
    bool state = read();
    if (state != m_state)
    {
        m_state = state;
