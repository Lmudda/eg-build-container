...
// transfer is in progress. When it completes, the code will start another transfer with the new data.
// RX runs continuously on a small circular RX buffer. Any data received is copied into an RXData 
// structure and emitted as an event. There is no equivalent of an accumulating RX buffer from which a 
...
private:
    ...
    // which all result in calls to RxEventCallback().
