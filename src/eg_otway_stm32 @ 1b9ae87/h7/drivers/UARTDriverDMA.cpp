...
, m_tx_buffer{tx_buffer}
{
    ...
    if (HAL_UARTEx_ReceiveToIdle_DMA(&m_uart_handle, &m_rx_buffer[0], RxBufferSize) != HAL_OK)
    {
        Error_Handler();
    }
}
...
void UARTDriverDMABase::write(const uint8_t* data, uint16_t length)
{
    ...
    // Just drop data if it doesn't fit.
...
void UARTDriverDMABase::TxCpltCallback()
{
    ...
    if (m_tx_buffer.size() > 0)
    {
        ...
        if (HAL_UART_Transmit_DMA(&m_uart_handle, m_tx_block.buffer, m_tx_block.length) != HAL_OK)
        {
            m_on_error.emit();
        }
    }
...
void UARTDriverDMABase::RxEventCallback(uint16_t pos)
{
    ...
    // The RX buffer is used in circular mode.
...
{
	// TODO: Error code is available if we wanted to pass it on.
	m_on_error.emit();

	// TODO: Need to check that this is robust in all cases.
	if (m_uart_handle.RxState == HAL_UART_STATE_READY)
	{
		// Attempt to restart the receive. This will reset the error code.
		if (HAL_UARTEx_ReceiveToIdle_DMA(&m_uart_handle, &m_rx_buffer[0], RxBufferSize) != HAL_OK)
		{
...
void UARTDriverDMABase::uart_init()
{
    ...
    // The interrupt priorities of the DMA TX, DMA RX and Idle Line can all be set to the same value. Does this matter?
    UARTHelpers::irq_enable(m_conf.uart, m_conf.uart_prio);
...
void UARTDriverDMABase::tx_dma_init()
{
    ...
    m_tx_dma_handle.Init.FIFOMode            = DMA_FIFOMODE_DISABLE;
    ...
    DMAHelpers::irq_enable(m_conf.tx_dma, m_conf.tx_prio);
...
void UARTDriverDMABase::rx_dma_init()
{
    ...
    m_rx_dma_handle.Init.FIFOMode            = DMA_FIFOMODE_DISABLE;
    ...
    DMAHelpers::irq_enable(m_conf.rx_dma, m_conf.rx_prio);
