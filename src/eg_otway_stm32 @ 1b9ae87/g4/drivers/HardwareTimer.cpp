...
: m_conf{conf}
{
    ...
    // Note this can't be used with advanced timers TIM1, TIM8 and TIM20 as they have multiple
    ...
    TIMHelpers::irq_enable(m_conf.tim, TIMHelpers::IRQType::Global, GlobalsDefs::DefPreemptPrio);
}
...
void HardwareTimer::change_time_period_ms(uint32_t period_ms)
{
    ...
    // Set clock to run at 1000Hz (i.e. 1ms)
    m_htim.Init.Prescaler = __HAL_TIM_CALC_PSC(TIMHelpers::SimpleTimers::get_input_freq(), 10'000);
    m_htim.Init.Period    = period_ms * 10;
...
uint32_t HardwareTimer::get_duration_left_ms()
{
    ...
    if (expiry_cnt >= current_cnt)
    {
        ...
        uint64_t remaining_ms = uint64_t{m_htim.Init.Prescaler} * 1000U * (expiry_cnt - current_cnt) / clock_freq;
