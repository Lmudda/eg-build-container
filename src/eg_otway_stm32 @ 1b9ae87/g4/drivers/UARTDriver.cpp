...
, m_tx_buffer{tx_buffer}
{
    ...
    m_huart.TxCpltCallback = &UARTDriverBase::TxCpltCallback;
    m_huart.RxCpltCallback = &UARTDriverBase::RxCpltCallback;
    m_huart.ErrorCallback  = &UARTDriverBase::ErrorCallback;
    ...
    HAL_UART_Receive_IT(&m_huart, &m_rx_buffer[0], 8);
}
...
{
    RXData rx{};
    rx.length = kRxBufferSize;
    std::memcpy(&rx.data[0], &m_rx_buffer[0], kRxBufferSize);
    m_on_rx_data.emit(rx);

    HAL_UART_Receive_IT(&m_huart, &m_rx_buffer[0], kRxBufferSize);
}
...
// for context.
void UARTDriverBase::TxCpltCallback(UART_HandleTypeDef *huart)
{
...
void UARTDriverBase::RxCpltCallback(UART_HandleTypeDef *huart)
{
    auto handle = reinterpret_cast<UARTDriverBase::UARTHandle*>(huart);
...
void UARTDriverBase::ErrorCallback(UART_HandleTypeDef *huart)
{
    auto handle = reinterpret_cast<UARTDriverBase::UARTHandle*>(huart);
